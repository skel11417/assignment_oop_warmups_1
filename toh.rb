...
class Toh
  def set_num_rings(num)
    @num_rings = num
    puts "set_num_rings works"
  end

  def num_rings
    puts "num_rings works!"
    return @num_rings
  end
  ...
  def set_rods(num_rings)
    @rods = Array.new(3) {Array.new(@num_rings, 0)}
    @rods[0] = (1..@num_rings).to_a
  end
